...
// Redis servers
const redisSend = redis.createClient();
const redisReceive = redis.createClient();

// Rod IDs to swap
const valid_rod_ids = [1, 2, 4, 6]

redisSend.on('error', (err) => {
...
function handleIncomingData(jsonData) {
  ...
  if (rod_index > -1) {
    ...
    // Push json to socket
    redisSend.lpush('task_queue', newBuffer, (err, reply) => {
      if (err) {
...
redisReceive.on("message", (channel, message) => {
  const data = JSON.parse(message);
  console.log(`Received from ${channel}:`, data);
});

redisReceive.subscribe("playerUpdate");
